...
See [examples/routetable_subnet_association](examples/routetable_subnet_association) for a full working example.
...
Post-deploy tests in `tests/post_deploy_functional/` and `tests/post_deploy_functional_readonly/` target `examples/routetable_subnet_association` via an explicit folder constant in each `main_test.go`. Adding another example requires a new test entry point or updating that constant; it is not picked up automatically.
